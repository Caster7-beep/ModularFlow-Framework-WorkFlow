...
const CodeBlockNode: React.FC<CodeBlockNodeProps> = ({ data, selected }) => {
  ...
  // 标题与徽标（语言），允许换行，避免文字溢出
  const label: string = useMemo(() => (cfg.label || data.label || '代码块').toString(), [cfg.label, data.label]);
  const codeType: string = useMemo(
    () => (cfg.code_type || cfg.language || 'python').toString().toUpperCase(),
    [cfg.code_type, cfg.language]
  );
  ...
  return (
    ...
    >
      ...
      {/* 极简卡片：图标 + 标题 + 一行徽标（语言）。名称可换行，节点随内容伸缩 */}
      <div
        ref={cardRef}
