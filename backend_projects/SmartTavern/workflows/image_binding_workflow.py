...
# 注册工作流
def register_workflow(api_gateway):
    """注册图像绑定工作流"""
    workflow = ImageBindingWorkflow()
    
    api_gateway.register_async_function(
        "image_binding.embed_files_to_image",
        workflow.embed_files_to_image,
        "将文件嵌入到PNG图片中"
    )
    
    api_gateway.register_async_function(
        "image_binding.extract_files_from_image",
        workflow.extract_files_from_image,
        "从PNG图片中提取文件"
    )
    
    api_gateway.register_async_function(
        "image_binding.get_embedded_files_info",
        workflow.get_embedded_files_info,
        "获取PNG图片中嵌入的文件信息"
    )
    
    api_gateway.register_async_function(
        "image_binding.is_image_with_embedded_files",
        workflow.is_image_with_embedded_files,
        "检查PNG图片是否包含嵌入文件"
    )
    
    api_gateway.register_async_function(
        "image_binding.get_file_type_tags",
        workflow.get_file_type_tags,
        "获取所有支持的文件类型标签"
    )
    
    api_gateway.register_async_function(
        "image_binding.test",
        workflow.test_image_binding,
        "测试图像绑定模块的功能"
    )